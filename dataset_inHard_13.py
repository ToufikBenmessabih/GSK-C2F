...
class AugmentDataset(torch.utils.data.Dataset):
    ...
    def getitem(self, index):  # Try to use this for debugging purpose
        ...
        #print(image_path)
        feats = np.load(image_path)
        #print(feats.shape)

        elements = feats.T
        #print(elements.shape)
        end_frame_real = elements.shape[1]


        if self.args.feature_size == 256:
            elements = elements.T
        count = 0
        end_frame = min(end_frame, end_frame_real, st_frame + (self.max_frames_per_video * self.chunk_size))
        ...
        for i in range(aug_start_frame, aug_end_frame, aug_chunk_size):
            ...
            key = elements[:, i: end]
            #print('augmentation: ', i, end, aug_chunk_size)
            #print('key: ', key.shape)
